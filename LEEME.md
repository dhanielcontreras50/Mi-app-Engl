...
estilos/estilo.css        papel y tinta; azul para el inglés, vino para el español
codigo/db.js              IndexedDB: tarjetas, programacion, intentos, pasajes, config
codigo/srs.js             SM-2. No sabe qué hay dentro de una tarjeta
codigo/llm.js             genera tarjetas, valida gramática, califica speaking
codigo/voz.js             TTS y reconocimiento de voz del celular
codigo/app.js             sesión, generación, progreso, ajustes
sw-ingles.js                 cache del armazón para abrir sin datos
```

La separación importante: **`codigo/srs.js` nunca mira dentro de `carga`.** Pide las
...
## Agregar tarjetas a mano

En **Agregar** hay dos modos: *Generar con IA* y *Escribir a mano*.
El manual no consume API. En vocabulario, una palabra guarda dos tarjetas
(una por dirección), igual que las generadas.

El mazo inicial (`mazo-inicial.json`) se carga desde **Ajustes → Importar**.
Trae 332 tarjetas escalonadas a 14 nuevas por día durante 24 días, para que
no te caigan todas encima el primer día.
