...
const CAMPOS = {
  vocab: [
    ['en', 'Palabra o frase en inglés', 'input'],
    ['es', 'Significado en español', 'input'],
    ['ejemplo', 'Frase de ejemplo en inglés', 'input'],
    ['ipa', 'Pronunciación (opcional)', 'input'],
  ],
  gramatica: [
    ['frase', 'Frase con ___ donde va el espacio', 'input'],
    ['solucion', 'Respuesta correcta', 'input'],
    ['aceptadas', 'Otras respuestas válidas, separadas por coma', 'input'],
    ['explicacion', 'Explicación en español', 'textarea'],
  ],
  lectura: [
    ['titulo', 'Título del texto', 'input'],
    ['texto', 'Texto en inglés', 'textarea'],
    ['pregunta', 'Pregunta sobre el texto', 'input'],
    ['op1', 'Opción 1', 'input'],
    ['op2', 'Opción 2', 'input'],
    ['op3', 'Opción 3', 'input'],
    ['op4', 'Opción 4', 'input'],
    ['correcta', 'Número de la opción correcta (1 a 4)', 'input'],
  ],
  speaking: [
    ['consigna', 'Lo que debes decir, en español', 'input'],
    ['criterio', 'Qué se evalúa', 'input'],
    ['objetivo', 'Estructura que debe aparecer', 'input'],
  ],
};

function pintarCamposMano() {
  const tipo = $('#tipo').value;
  const c = $('#camposMano');
  c.innerHTML = '';
  CAMPOS[tipo].forEach(([clave, texto, etiqueta]) => {
    const l = el('label', null, texto);
    l.htmlFor = `mano-${clave}`;
    c.appendChild(l);
    const campo = el(etiqueta);
    campo.id = `mano-${clave}`;
    if (etiqueta === 'textarea') campo.rows = 5;
    if (clave === 'en' || clave === 'ejemplo') campo.autocapitalize = 'none';
    c.appendChild(campo);
  });
}

function leerCampo(clave) {
  const n = document.getElementById(`mano-${clave}`);
  return n ? n.value.trim() : '';
}

async function guardarAMano() {
  const tipo = $('#tipo').value;
  const salida = $('#resultadoMano');
  salida.innerHTML = '';
  const avisar = (msg, ok) => salida.appendChild(el('div', ok ? 'aviso ok' : 'aviso', msg));

  const tarjetas = [];
  const nueva = (carga, extra = {}) => {
    const t = { id: db.uuid(), tipo, carga, creada: Date.now(), origen: 'mano', ...extra };
    tarjetas.push(t);
    return t;
  };

  if (tipo === 'vocab') {
    const en = leerCampo('en'), es = leerCampo('es');
    if (!en || !es) return avisar('Faltan la palabra en inglés y su significado.');
    const carga = { en, es, ejemplo: leerCampo('ejemplo'), ipa: leerCampo('ipa') };
    const notaId = db.uuid();
    nueva(carga, { notaId, direccion: 'en-es' });
    nueva(carga, { notaId, direccion: 'es-en' });
  } else if (tipo === 'gramatica') {
    const frase = leerCampo('frase'), solucion = leerCampo('solucion');
    if (!frase || !solucion) return avisar('Faltan la frase y la respuesta correcta.');
    if (!frase.includes('___')) return avisar('La frase debe llevar ___ donde va el espacio.');
    nueva({
      frase, solucion,
      aceptadas: leerCampo('aceptadas').split(',').map((x) => x.trim()).filter(Boolean),
      explicacion: leerCampo('explicacion'),
    });
  } else if (tipo === 'lectura') {
    const texto = leerCampo('texto'), pregunta = leerCampo('pregunta');
    const opciones = ['op1', 'op2', 'op3', 'op4'].map(leerCampo).filter(Boolean);
    const correcta = parseInt(leerCampo('correcta'), 10) - 1;
    if (!texto || !pregunta) return avisar('Faltan el texto y la pregunta.');
    if (opciones.length < 2) return avisar('Escribe al menos dos opciones.');
    if (!(correcta >= 0 && correcta < opciones.length)) {
      return avisar(`El número de la opción correcta debe estar entre 1 y ${opciones.length}.`);
    }
    const pasaje = { id: db.uuid(), titulo: leerCampo('titulo'), texto, nivel: $('#nivel').value, tema: '' };
    await db.poner('pasajes', pasaje);
    nueva({ pasajeId: pasaje.id, pregunta, opciones, correcta });
  } else {
    const consigna = leerCampo('consigna');
    if (!consigna) return avisar('Falta la consigna.');
    nueva({ consigna, criterio: leerCampo('criterio') || 'Corrección general', objetivo: leerCampo('objetivo') });
  }

  await db.ponerVarios('tarjetas', tarjetas);
  await db.ponerVarios('programacion', tarjetas.map((t) => srs.programacionInicial(t.id)));

  // Deja los campos listos para la siguiente, sin perder el ritmo.
  pintarCamposMano();
  avisar(`Guardada. ${tarjetas.length === 2 ? 'Quedaron dos tarjetas: una por dirección.' : ''} Van ${tarjetas.length}.`, true);
  actual = null;
  await refrescarContador();
}

document.querySelectorAll('#modos .modo').forEach((b) => {
  b.addEventListener('click', () => {
    const mano = b.dataset.modo === 'mano';
    document.querySelectorAll('#modos .modo').forEach((x) => x.classList.toggle('activa', x === b));
    $('#panelIA').classList.toggle('oculto', mano);
    $('#panelMano').classList.toggle('oculto', !mano);
    if (mano) pintarCamposMano();
  });
});

$('#tipo').addEventListener('change', () => {
  if (!$('#panelMano').classList.contains('oculto')) pintarCamposMano();
});

$('#btnGuardarMano').addEventListener('click', () => {
  guardarAMano().catch((e) => {
    $('#resultadoMano').innerHTML = '';
    $('#resultadoMano').appendChild(el('div', 'aviso', `No se pudo guardar: ${e.message}`));
  });
});
...
async function inicio() {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('sw-ingles.js').catch(() => {});
  }
}
